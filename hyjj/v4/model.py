...
if __name__ == '__main__123':
    from hyjj.v4.dataset import HyjjDataset

    dataset = HyjjDataset(r'E:\BaiduNetdiskDownload\hyjj_signal_demod\train_data',
                          is_train=False)
    ...
    for i in range(len(dataset)):
        i_data, q_data, code_series, amr_label, code_width = dataset[i]
        bat_x = torch.from_numpy(np.expand_dims(np.array([i_data, q_data], dtype=np.float32), axis=0))
        pred_amr, pred_cw = model(bat_x)

    # with torch.no_grad():
    #     for i in range(128, 3125):
    #         bat_x = torch.randn([4, 2, i], dtype=torch.float32)
    #         pred_amr, pred_cw = model(bat_x)
