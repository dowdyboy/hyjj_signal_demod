...
class BiLSTMModel2AMR(nn.Module):

    def __init__(self, feature_extractor, lstm_input_size, hidden_size, num_layers=2, num_classes=10):
        super(BiLSTMModel2AMR, self).__init__()
        self.feature_extractor = feature_extractor
        self.lstm_input_size = lstm_input_size
...
class BiLSTMModel2CW(nn.Module):
    def __init__(self, feature_extractor, lstm_input_size, hidden_size, num_layers=2):
        super(BiLSTMModel2CW, self).__init__()
        self.feature_extractor = feature_extractor
        self.lstm_input_size = lstm_input_size
...
class BiLSTMModel2Demod(nn.Module):

    def __init__(self, feature_extractor, lstm_input_size, hidden_size, num_layers=2, num_classes=10):
        super(BiLSTMModel2Demod, self).__init__()
        self.feature_extractor = feature_extractor
        self.lstm_input_size = lstm_input_size

        self.lstm_demod = nn.LSTM(input_size=self.lstm_input_size,
                                hidden_size=hidden_size,
                                num_layers=num_layers,
                                bidirectional=True,
                                batch_first=True)
        self.attention_demod = Attention(hidden_size * 2)
        self.fc_demod = nn.Linear(hidden_size * 2, num_classes)

    def forward(self, x):
        x = self.feature_extractor(x)
        x = x.permute(0, 2, 1)

        x_demod, _ = self.lstm_demod(x)
        x_demod = self.attention_demod(x_demod)
        x_demod = self.fc_demod(x_demod)
