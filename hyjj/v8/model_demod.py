...
class UNET_1D(nn.Module):
    def __init__(self, input_dim, output_dim, layer_n, kernel_size, depth, ):
        ...
        self.outconv = nn.Conv1d(self.layer_n, self.output_dim, kernel_size=self.kernel_size, stride=1, padding=3)
    ...
    def forward(self, x):
        ...
        pool_x1 = self.AvgPool1D1(x)  # 5倍下采样
        pool_x2 = self.AvgPool1D2(x)  # 25倍下采样
        pool_x3 = self.AvgPool1D3(x)  # 125倍下采样

        #############Encoder#####################

        out_0 = self.layer1(x)
        out_1 = self.layer2(out_0)  # 进行了一次下采样，5倍下采样
...
if __name__ == '__main__':
    ...
    model = UNET_1D(2, num_class, 64, 7, 3)
    bat_x = torch.rand([128, 2, 1000])
    bat_y = torch.rand([128, 1000]) * num_class
    bat_y = torch.tensor(bat_y, dtype=torch.long)
