...
def gen_demod_data_aug(i_data, q_data, sample_len, code_series, code_width, sample_type='rand', ):
    data_len = len(i_data)
    Fs = 20e6  # 采样率20MHz
    code_width = float(code_width / 1e6)
    smp_per_code = int(round(code_width * Fs))
    ...
    return i_data, q_data, pos_data, cs_data


def gen_demod_data_aug_infer(i_data, q_data, sample_len, code_width, ):
    data_len = len(i_data)
    Fs = 20e6  # 采样率20MHz
    code_width = float(code_width / 1e6)
    smp_per_code = int(round(code_width * Fs))
...
class HyjjDemodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        i_data, q_data, code_series, amr_label, code_width = self.dataset[idx]
        i_data, q_data, spectrum = process_nco_signal(i_data, q_data, code_series.shape[0], code_series, code_width)
        i_data, q_data, pos_data, cs_data = gen_demod_data_aug(i_data, q_data, self.data_len, code_series, code_width)
        ...
        if self.position_code:
            return np.array([i_data, q_data, pos_data], dtype=np.float32), cs_data
        else:
            return np.array([i_data, q_data], dtype=np.float32), cs_data
...
if __name__ == '__main__':
    ...
    ds = HyjjDemodDataset(train_base_ds, 1024, True)
