...
from hyjj.v6.dataset import HyjjDemodDataset
from hyjj.v6.model import CNNFeatureExtractor, BiLSTMModel2Demod
...
parser.add_argument('--data-len', type=int, default=256, help='data len')
## optimizer
...
parser.add_argument('--output-dir', type=str, default='./output_v6_demod', help='out dir')
...
def build_data():
    ...
    for model_name in args.model_type:
        train_ds = HyjjDemodDataset(args.train_data_dir, model_name, args.data_len, )
        # train_ds = HyjjDemodDataset(args.val_data_dir, model_name, args.data_len, )
        train_dl = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, )
        val_ds = HyjjDemodDataset(args.val_data_dir, model_name, args.data_len, )
...
def build_model():
    ...
    for model_name in args.model_type:
        model_list.append(
            BiLSTMModel2Demod(
                CNNFeatureExtractor(hidden_chanel=32, num_layer=3, drop_rate=0.1),
                256, 256, 1, num_class_map[model_name]
            )
        )
    return model_list
