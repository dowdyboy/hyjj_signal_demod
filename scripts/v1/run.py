...
from hyjj.v1.model import CNNFeatureExtractor, BiLSTMModel2
...
def main(to_pred_dir, result_save_path):
    ...
    # 初始化模型
    device = 'cuda'
    amr_model_path = 'output_all/output_v1_2412162042/checkpoint/chk_best_amr_step_89_1265/model_0.pt'
    cw_model_path = 'output_all/output_v1_2412162042/checkpoint/chk_best_cw_step_20_1265/model_0.pt'
    ...
    for filename in test_file_lst:
        ...
        bat_x = torch.from_numpy(np.array([df.iloc[:, 0].to_numpy() , df.iloc[:, 1].to_numpy()], dtype=np.float32))
        ...
        with torch.no_grad():
            ...
            pred_y_cw = float(pred_y_cw[0].cpu().numpy()[0])
