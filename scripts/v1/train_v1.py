...
from hyjj.v1.dataset import HyjjDataset, HyjjAMRDataset, HyjjAMRCWDataset
from hyjj.v1.model import BiLSTMModel, CNNFeatureExtractor, BiLSTMModel2
# from hyjj.v1.model2 import UNET_1D
...
parser.add_argument('--sample-len', type=int, default=1024, help='data sample len')
## optimizer
...
parser.add_argument('--output-dir', type=str, default='./output_v1', help='out dir')
...
def build_data():
    ...
    # train_ds = HyjjAMRDataset(train_base_ds, args.sample_len, sample_type='rand', )
    train_ds = HyjjAMRCWDataset(train_base_ds, args.sample_len, sample_type='rand', )
    train_dl = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, )
    # val_ds = HyjjAMRDataset(val_base_ds, args.sample_len, )
...
def evaluate_model(model, data_loader, criterion_amr, criterion_cw, device):
    ...
    with torch.no_grad():
        for inputs, labels_amr, labels_cw in data_loader:
            ...
            all_cw_predictions.extend(torch.squeeze(outputs_cw, dim=1).cpu().numpy())
